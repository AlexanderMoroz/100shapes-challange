		...
		<Flex sx={{ listStyle: 'none' }} p={0}>
			{cards.map(({ amount, id }) => (
				...
				>
					...
					<MotionBox layoutId={`card-profile-${id}`} bg="background" sx={{ opacity: 1, height: 16, borderRadius: '50%', alignSelf: 'flex-end', '& svg': { m: '-1px' }}}><CgProfile size={18} color={theme.colors.text} /></MotionBox>
				</MotionBox>
			))}
		</Flex>
	</Box>
